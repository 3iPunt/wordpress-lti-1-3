...
// +----------------------------------------------------------------------+
//
// $Id: UtilsPropertiesWP.php,v 1.1 2007/12/12 11:53:56 jmassaguer Exp $
...
class UtilsPropertiesWP {
  ...
  private   $properties = array(); // associative array of key => Properties_Section_Property_WP object references.
  private   $loaded_properties_filename = "none"; // name of the loaded properties file
  private   $environment_key = null; // Environment key to override configuration file
  private   $sourcePathByDefault = "none"; // name of the default configuration file (for exception text)
	private   $provideDebugInformation = false;
	
  const WHITE_SPACE_CHARS = " \t\r\n\x0C"; // 0x0c == \f (form feed)

  /**
   * Returns name of the config file specified by the $envKey.
   * If not defined it takes the default value.
   * It does not check for existence of the file itself.
   *
   * @param unknown_type $sourcePathByDefault
   * @param unknown_type $envKey
   * @return unknown
   */
  protected static function getConfigFilename($sourcePathByDefault, $envKey) {
  	...
  	if ($result == null) {
  		$result = $sourcePathByDefault;
  	}
  	return $result;
  }
  
  public function __construct($sourcePathByDefault = null, $envKey = null) {
  	$this->environment_key = $envKey;
  	$this->sourcePathByDefault = $sourcePathByDefault;
  	
  	$sourcePathToUse = self::getConfigFilename($sourcePathByDefault, $envKey);
    $this->loadFromPath($sourcePathToUse);
    $this->loaded_properties_filename = $sourcePathToUse;
    return $this;
  }

	public function setDebugInformation($provideDebugInformation) {
		$this->provideDebugInformation = $provideDebugInformation;
	}
	
	/** Returns either the original location of the configuration file or null if debug information is not active. */
	public function getFilename() {
		if ($this->provideDebugInformation) {
			return $this->loaded_properties_filename;
		} else {
			return null;
		}
	}
  ...
  public function setProperty($key, $value) {
    //Properties_Section_Property_WP::testKey($key);
    //Properties_Section_Property_WP::testValue($key);
    if (array_key_exists($key, $this->properties)) {
      $this->properties[$key]->setValue($value);
    }
    else {
      $p = new Properties_Section_Property_WP($key, $value);
  ...
  /**
   * Loads properties from a file name.
   */
  public function loadFromPath($sourcePathToUse) {
  	$sections = array();
  	$properties = array();
    $individualFiles = explode(",", $sourcePathToUse);
    foreach ($individualFiles as $individualFile) {
    	$individualFile = trim($individualFile);
    	if ($individualFile != "") {
	  		if (!file_exists($individualFile)) {
	  			throw new Exception("The file ".$individualFile." does not exist. Path can be defined by environment variable " . $this->environment_key . " and defaults to " . $this->sourcePathByDefault);
	    	}
		  	$source= file_get_contents($individualFile);
				if($source === FALSE){
					throw new InvalidArgumentException("Cannot read configuration file $sourcePathToUse");
				}
		    $lines = preg_split('/(\\r\\n|\\n|\\r)/', $source); // DOS: \r\n	UNIX: \n	MAC: \r
		    ...
		    for ($i = 0; $i < count($lines); $i++) {      
		      ...
		      if (strlen($line) == 0) {
		        if ($i < count($lines) -1) { // don't store last blank.
		          $p = new Properties_Section_Blank_WP();
		          array_push($sections, $p);
		        }
		      }
		
		      // handle comments
		      elseif (preg_match('/^(#|!)(\\s*)(.*)$/', $line, $matches)) {
		        $p = new Properties_Section_Comment_WP($matches[3], $matches[1], $matches[2]);
		        array_push($sections, $p);
		      }
		      ...
		      // handle properties or multiline sections.
		        ...
		        // handle properties
		          ...
		          $p = new Properties_Section_Property_WP($line);
		          $key = $p->getKey();
		          if (array_key_exists($key, $properties)) {
		            // Property name already exists.
		            array_push($sections, new Properties_Section_Comment_WP("WARNING: The following previously encountered property was commented out:\n$line"));
		          }
		          else {
		            $properties[$key] = $p;
		            array_push($sections, $p);
		    ...
		    if (array_key_exists(CONFIGVALUE_LOW_PRECEDENCE_LOCATION, $properties)) {
		    	$this->loadFromPath($properties[CONFIGVALUE_LOW_PRECEDENCE_LOCATION]->getValue());
		    }
		    $this->sections = array_merge($this->sections, $sections);
		    $this->properties = array_merge($this->properties, $properties);
		    if (array_key_exists(CONFIGVALUE_HIGH_PRECEDENCE_LOCATION, $properties)) {
		    	$this->loadFromPath($properties[CONFIGVALUE_HIGH_PRECEDENCE_LOCATION]->getValue());
		    }
	    }
    }
  }
  ...
  public function store($file, $header = null) {
    ...
    if (isset($header) && strlen($header)) {
      $comment = new Properties_Section_Comment_WP($header, false);
    ...
    $this->loaded_properties_filename = $file;
  }
 ...
 /**
   * Gets a property value.
   * Returns a string , fails if the key doesn't exist
   *
   * @param string key
   * @return string
   * @throw InvalidArgumentException
   */
  public function getExistingProperty($key){
   	$property= $this->getProperty($key);
  	if(!isset($property) || $property == null) {
  		$propertiesLocation = $this->getFilename();
  		if ($propertiesLocation != null) {
	  		$exceptionText = "The property ".$key." does not exist in file ".$this->loaded_properties_filename.". File path can be defined by environment variable " . $this->environment_key;
  		} else {
	  		$exceptionText = "The property ".$key." does not exist";
  		}
  ...
  public function getPropertyAsBoolean($key, $defaultValue) {
  	$resultString = $this->getProperty($key);
		if ($resultString == null) {
			return $defaultValue;
		} else {
			if (strpos($resultString, "true") !== false) {
				$result = true;
			} else if (strpos($resultString, "false") !== false) {
				$result = false;
			} else {
	  		if ($propertiesLocation != null) {
		  		$exceptionText = "The property ".$key." in file ".$this->loaded_properties_filename . " contains an invalid value " . $resultString . ": not a boolean . File path can be defined by environment variable " . $this->environment_key;
	  		} else {
		  		$exceptionText = "The property ".$key." contains an invalid value " . $resultString . ": not a boolean";
	  		}
	  		throw new Exception($exceptionText);
			}
			return result;
 ...
 /**
   * Gets a property value, ensuring that it points to an existing key.
   * Returns a string , fails if the key doesn't exist
   *
   * @param string key
   * @return string
   * @throw InvalidArgumentException
   */
  public function getExistingClassProperty($key) {
  	$className = $this->getExistingProperty($key);
  	if (!@fclose(@fopen($className, 'r', true))) {
  		$propertiesLocation = $this->getFilename();
  		if ($propertiesLocation != null) {
	  		$exceptionText = "The property ".$key." in file ".$this->loaded_properties_filename . " points to " . $className . ", which does not exist as a PHP class. Include path: " . get_include_path() . " . File path can be defined by environment variable " . $this->environment_key;
  		} else {
	  		$exceptionText = "The property ".$key." points to a class which does not exist as a PHP class";
  		}
  		throw new Exception($exceptionText);
  	}
  	return $className;
	...
	/**
	 * Parses value of property as a list of properties detached by comma.
	 * Result elements are trimmed to remove spaces surrounding the comma.
	 * @return Array of values
	 */
	public function getExistingPropertiesDetachedByComma($key) {
		$propertiesToDetachByComma = $this->getExistingProperty($key);
		$propertiesDetachedByComma = explode(",", $propertiesToDetachByComma);
		foreach ($propertiesDetachedByComma as $key => $value) {
			$propertiesDetachedByComma[$key] = trim($value);
			if ($value=="") {
				unset ($propertiesDetachedByComma[$key]);
			}
		}
		return $propertiesDetachedByComma;
	}
   ...
   * @throw InvalidArgumentException
   */
  public function getClassProperty($key) {
  	$className = $this->getProperty($key);
  	if(isset($property) && $property != null) {
	  	if (!@fclose(@fopen($className, 'r', true))) {
			$propertiesLocation = $this->getFilename();
	  		if ($propertiesLocation != null) {
   ...
   * @return boolean
   */
  public function equals(UtilsPropertiesWP $other) {
  ...
  public function remove($key) {
    ...
    if (array_key_exists($key, $this->properties)) {
      ...
      for ($i = 0; $i < count($this->sections); $i++) {
        if (($this->sections[$i] instanceof Properties_Section_Property_WP) && ($this->sections[$i]->getKey() == $key)) {
          $splice_from = $splice_to = $i;
          if ($i > 0) {
            $previous_section_class = get_class($this->sections[$i - 1]);
            if ($previous_section_class != 'Properties_Section_Property_WP') {
              for ($j = $i - 1; $j >= 0; $j--) {
 ...
 */
interface Properties_ISection_WP {
...
/****************************** End of interface Properties_ISection_WP ******************************/
 ...
 */
class Properties_Section_Blank_WP implements Properties_ISection_WP {
...
/****************************** End of class Properties_Section_Blank_WP ******************************/
 ...
 */
class Properties_Section_Comment_WP implements Properties_ISection_WP {
...
/****************************** End of class Properties_Section_Comment_WP ******************************/
 ...
 */
class Properties_Section_Property_WP implements Properties_ISection_WP {
  ...
  public function __construct() {
    ...
    case 1:
      $line = func_get_arg(0);
      if (isset($line)) {
        $line = ltrim($line, UtilsPropertiesWP::WHITE_SPACE_CHARS);
      }
...
/****************************** End of class Properties_Section_Property_WP ******************************/
